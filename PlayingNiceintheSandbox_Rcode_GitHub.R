...
setwd("E:/Nuffield DPhil/Experiment paper 2/Rstudio/Data")
...
# Placetrust 
table(prehandling.data$placetrust)
setDT(prehandling.data)[, placetrust:= placetrust[!is.na(placetrust)][1L] , by = .(session, Subject)]
table(prehandling.data$placetrust)

#honoured trust 
table(prehandling.data$honoured)
setDT(prehandling.data)[, honoured:= honoured[!is.na(honoured)][1L] , by = .(session, Subject)]
table(prehandling.data$honoured)


# Sendback of player 2 and send of player 1   
prehandling.data$player2_sendback = with(prehandling.data, 
                                         ifelse(treatment == 1 & player == 2, honoured, NA))
table(prehandling.data$player2_sendback)
setDT(prehandling.data)[, player2_sendback:= player2_sendback[!is.na(player2_sendback)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player2_sendback)


prehandling.data$player1_send = with(prehandling.data, 
                                     ifelse(treatment == 1 & player == 2, placetrust, NA))
table(prehandling.data$player1_send)
setDT(prehandling.data)[, player1_send:= player1_send[!is.na(player1_send)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player1_send)


#Sendback of player 2 ingroup and send of player 1 ingroup 
prehandling.data$player2_sendback_in = with(prehandling.data, 
                                            ifelse(part == 4 & player == 2, honoured_ingroup, NA))
table(prehandling.data$player2_sendback_in)
setDT(prehandling.data)[, player2_sendback_in:= player2_sendback_in[!is.na(player2_sendback_in)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player2_sendback_in)


prehandling.data$player2_send_in = with(prehandling.data, 
                                        ifelse(part == 4 & player == 2, placetrust_ingroup_player1, NA))
table(prehandling.data$player2_send_in)
setDT(prehandling.data)[, player2_send_in:= player2_send_in[!is.na(player2_send_in)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player2_send_in)

#Sendback of player 2 outgroup and send of player 1 outgroup

prehandling.data$player2_sendback_out = with(prehandling.data, 
                                             ifelse(part == 5 & player == 2, honoured_outgroup, NA))
table(prehandling.data$player2_sendback_out)
setDT(prehandling.data)[, player2_sendback_out:= player2_sendback_out[!is.na(player2_sendback_out)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player2_sendback_out)


prehandling.data$player2_send_out = with(prehandling.data, 
                                         ifelse(part == 5 & player == 2, placetrust_outgroup_player1, NA))
table(prehandling.data$player2_send_out)
setDT(prehandling.data)[, player2_send_out:= player2_send_out[!is.na(player2_send_out)][1L] , by = .(session, Subject)]
table(prehandling.data[which(prehandling.data$treatment ==5),]$player2_send_out)
...
setwd("E:/Nuffield DPhil/Experiment paper 2/Rstudio/Data") #set working directory
...
#Does higher placed trust increase honoured trust?
Trustmodel <- lm(formula = honoured ~ placetrust +  Age + Sex + Friends + Gametheory + netherlands, data = working.data)
summary(Trustmodel)

trustmodel2 <- lm(formula = trustworth_general ~ trust_general + Age + Sex + Friends + Gametheory + netherlands, data = working.data)
summary(trustmodel2)
...
working.data$relative_send_p1 = working.data$player1_send / 10
working.data$relative_sendback_p2 = working.data$player2_sendback / (working.data$player1_send + 10)

table(working.data$relative_send_p1)
table(working.data$relative_sendback_p2)

CR1 = lm(relative_sendback_p2 ~ relative_send_p1 + Age + Sex + Friends + Gametheory + netherlands, data = working.data[which(working.data$Period ==1),])
summary(CR1)
